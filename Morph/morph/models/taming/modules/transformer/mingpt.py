...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size, block_size, n_layer=12, n_head=8, n_embd=256,
                 embd_pdrop=0., resid_pdrop=0., attn_pdrop=0., n_unmasked=0, ori_block_size=256):
        super().__init__()
        config = GPTConfig(vocab_size=vocab_size, block_size=block_size,
                           embd_pdrop=embd_pdrop, resid_pdrop=resid_pdrop, attn_pdrop=attn_pdrop,
                           n_layer=n_layer, n_head=n_head, n_embd=n_embd,
                           n_unmasked=n_unmasked, ori_block_size=ori_block_size)
        ...
        # self.pos_emb = nn.Parameter(torch.zeros(1, config.ori_block_size, config.n_embd))
    ...
    def forward(self, idx, embeddings=None, targets=None):
        ...
        # print(embeddings.size(), token_embeddings.size())
        position_embeddings = self.pos_emb[:, :t, :] # each position maps to a (learnable) vector
        x = self.drop(token_embeddings + position_embeddings)
        # print('aa', position_embeddings.size(), token_embeddings.size(), idx.size(), embeddings.size())
        # position_embeddings = self.pos_emb[:, 1:, :] 
        # token_embeddings[:, 32:, :] = token_embeddings[:, 32:, :] + position_embeddings
...
@torch.no_grad()
def sample_new(model, c, steps, temperature=1.0, sample=False, top_k=None):
    """
    take a conditioning sequence of indices in x (of shape (b,t)) and predict the next token in
    the sequence, feeding the predictions back into the model each time. Clearly the sampling
    has quadratic complexity unlike an RNN that is only linear, and has a finite context window
    of block_size, unlike an RNN that has an infinite context window.
    """
    block_size = model.get_block_size() - c.shape[1]
    x = None
    model.eval()
    for k in range(steps):
        x_cond = x if x.size(1) <= block_size else x[:, -block_size:]  # crop context if needed
        logits, _ = model(idx=x_cond, embeddings=c)
        # pluck the logits at the final step and scale by temperature
        logits = logits[:, -1, :] / temperature
        # optionally crop probabilities to only the top k options
        if top_k is not None:
...
@torch.no_grad()
def sample_with_past_new(c, model, steps, temperature=1., sample_logits=True,
                     top_k=None, top_p=None, callback=None):
    # x is conditioning
    sample = None
    cond_len = c.shape[1]
    ...
    for n in range(steps):
        if callback is not None:
            callback(n)
        if past is None:
            logits, _, present = model.forward_with_past(idx=x, embeddings=c, past=past, past_length=(n+cond_len-1))
            past = [present]
        else:
            logits, _, present = model.forward_with_past(idx=x, past=past, past_length=(n+cond_len-1))
            past.append(present)
        logits = logits[:, -1, :] / temperature
        if top_k is not None:
            logits = top_k_top_p_filtering(logits, top_k=top_k, top_p=top_p)
