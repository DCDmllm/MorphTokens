...
from morph.models.taming.modules.util import SOSProvider
...
class Net2NetTransformer(pl.LightningModule):
    def __init__(self,
        ...
        if permuter_config is None:
            permuter_config = {"target": "morph.models.taming.modules.transformer.permuter.Identity"}
    ...
    def forward(self, x, c):
        ...
        target = z_indices
        # make the prediction
        logits, _ = self.transformer(idx=cz_indices[:, :-1], embeddings=c)
        # cut off conditioning outputs - output i corresponds to p(z_i | z_{<i}, c)
        logits = logits[:, c.shape[1]-1:]
        # print(logits.size(), z_indices.size())
